...
## Every one of them can say what it is made of

`--licences` prints the third-party licence notice of everything linked into that
binary or packaged beside it (`REQ-CON-013`,
[ADR-0083](../docs/adr/0083-the-notice-travels-inside-the-artifact.md)). The four
Rust plugins carry theirs **compiled in** with `include_str!`, because a `scratch`
image has no filesystem to read a file from; `oidc` carries its own as a resource
in the jar. Both are generated by [`tools/notices.py`](../tools/notices.py) from
what the artifact actually holds, committed as `THIRD-PARTY-NOTICES.txt`, and
regenerated by CI, which fails on any difference.

A new plugin here needs one line in that tool's artifact table and nothing else.
