...
| The upload's own bytes | scanned, then deleted with the spool file |
...
encode on a request thread that is already holding a ClamAV round trip, and
[ADR-0051](0051-broker-in-stage-0.md) moved the *other* derivatives off that
thread for precisely that reason.
