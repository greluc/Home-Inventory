...
| First-party plugins | **One of five, and installable** | [`plugins/webhook/`](../../plugins/webhook/) is written and declared in [`deploy/services.yaml`](../../deploy/services.yaml): one entry produces the container, its own segment, its section of the egress allowlist and its line in the list the core reads (`REQ-PLG-013`). `plugin-smtp`, `plugin-blobstore-s3`, `plugin-blobstore-nextcloud` and `plugin-oidc` are not written — so a deployment can receive a webhook and cannot send mail ([ADR-0072](../adr/0072-first-party-plugins-live-here.md)) |
