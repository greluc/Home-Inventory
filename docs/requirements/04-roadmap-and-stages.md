...
| Login | **Built** | TOTP and passkeys, re-confirmation, the session overview, service accounts, password reset. `SecondFactorIT`, `SecondFactorStepUpIT`, `PasskeyIT`, `SessionOverviewIT`, `ServiceAccountIT`, `PasswordResetIT`. **OIDC federation is built too** since 2026-09-21: the core half in `FederatedSignInIT` and `FederatedRegistrationIT`, the provider in [`plugins/oidc/`](../../plugins/oidc/), and the refusal that matters most — an identity nobody linked is never signed in on a matching address (`REQ-AUTH-006`, [ADR-0076](../adr/0076-a-federated-sign-in-creates-an-account-only-where-a-form-could.md)) |
...
| Events | **Partly** | The outbox, RabbitMQ and the `worker` role are stage 0 and built ([ADR-0051](../adr/0051-broker-in-stage-0.md)); the media queue dead-letters, and a notification that cannot be delivered is dead-lettered rather than retried for ever (`NotificationDeliveryIT`). Every event names itself since 2026-09-21 — `TenantScopedEvent.eventType()` against the registry in [`event-types.yaml`](../reference/event-types.yaml), held to the code by `EventTypeRegistryTest`, which is the catalogue a consumer contract would be written against. **A replay of it is not built**, and neither is the consumer-contract test |
| Plugin runtime | **Built** | The registry with manifests and digests, the per-tenant capability model beside the instance-level one, lifecycle and health, gRPC over mTLS with a pinned certificate per plugin, deadline, payload limit, bulkhead and circuit breaker around every call, the per-plugin network segment in [`deploy/services.yaml`](../../deploy/services.yaml), and the host channel a plugin may call back on ([ADR-0071](../adr/0071-the-core-answers-plugins-on-one-channel.md)). `PluginRuntimeIT`, `PluginCapabilityIT`, `InstanceCapabilityIT`, `PluginConsentIT`, `InstalledPluginsIT`, `HostChannelIT`. **Manifest signature verification too**, since 2026-09-22: the core verifies a detached signature over the manifest's bytes against the operator's key, offline, and `signed` is what it found rather than what the operator stated — three outcomes, since an altered manifest is not an unsigned one ([ADR-0085](../adr/0085-a-manifest-signature-is-checked-offline-and-an-image-is-not.md), `ManifestSignatureTest`, `ManifestSignatureIT`). The **artifact** half waits on there being artifacts: nothing is published to a registry, so there is no image signature to verify |
| First-party plugins | **Built** | [`plugins/webhook/`](../../plugins/webhook/), [`plugins/smtp/`](../../plugins/smtp/), [`plugins/blobstore-s3/`](../../plugins/blobstore-s3/), [`plugins/blobstore-nextcloud/`](../../plugins/blobstore-nextcloud/) and [`plugins/oidc/`](../../plugins/oidc/) are written and declared in [`deploy/services.yaml`](../../deploy/services.yaml): one entry each produces the container, its own segment, its section of the egress allowlist and its line in the list the core reads (`REQ-PLG-013`). The four in Rust share [`plugins/common/`](../../plugins/common/) — the mTLS identity, the `CONNECT` tunnel and the TLS at the far end of it, one HTTP/1.1 exchange over both, HMAC-SHA256 and the calendar arithmetic behind a UTC stamp — which ADR-0072 said would appear with the second plugin and grew with the third. `plugin-oidc` is the exception that decision named: Java, because ID-token validation is where an audited library outweighs an image size, and its own line in the memory budget since [ADR-0075](../adr/0075-the-one-jvm-plugin-has-its-own-line-in-the-budget.md) |
| Media | **Partly** | Reference counting, the fail-closed malware scan, derivatives and the signed URLs are built (`MediaListingIT`, `MediaScanIT`, `DerivativeGenerationIT`, `OrphanedBlobSweepIT`). ~~**Resumable upload (tus) is not**~~ — **built 2026-09-22** (`REQ-MED-008`, [ADR-0084](../adr/0084-an-upload-arrives-in-pieces-and-is-staged-where-the-volume-is.md)): the bytes are staged in the `blobstore` service so that `api` stays stateless, the offset a client resumes from is the staged file's length rather than a column, and the web client continues by itself after a dropped connection (`ResumableUploadIT`, `upload.test.ts`). *The 2026-09-14 survey said it was already there, and nothing in the tree had implemented it.* The **S3 and Nextcloud adapters are plugins** and are written, with the port they plug into wired at the same time: a tenant that grants one has its next bytes written there and none of its older ones moved (`TenantBlobStoreIT`, [ADR-0074](../adr/0074-granting-a-store-routes-and-does-not-move.md)) |
| API | **Partly** | `ETag`/`If-Match`, `Idempotency-Key`, the bulk surface, the response envelope, the keyset cursors, RFC 9457 everywhere and the generated OpenAPI document are built. **Server-sent events are built** since 2026-09-21 (`REQ-API-011`, `EventStreamIT`): `GET /api/v1/events` nudges an open view with a kind and nothing else, through Valkey so that it works with more than one `api` replica. **Webhooks are built** since 2026-09-21 as well (`REQ-API-010`, `WebhookTargetIT`): a tenant administrator subscribes a URL to event types from [`event-types.yaml`](../reference/event-types.yaml), the delivery is written in the transaction that made the change, and `plugin-webhook` signs it with that target's own secret ([ADR-0077](../adr/0077-a-call-may-carry-a-setting-the-tenant-did-not-configure.md), [ADR-0078](../adr/0078-a-webhook-carries-an-id-a-live-nudge-does-not.md)). **GraphQL is built** since 2026-09-21 (`REQ-API-006`, `GraphQlIT`): seven read-only queries, a `@cost` weight per field, a register of queries generated from the client, and a permission per resolver so a missing one costs a field rather than the query ([ADR-0079](../adr/0079-the-graphql-surface-is-registered-weighed-and-checked-per-field.md)). **The generated clients are built** since 2026-09-21 (`REQ-API-002`): TypeScript from `openapi-typescript` with `api.ts` reduced to the transport rules the document cannot describe, and Kotlin Multiplatform as `:api-client-kotlin`, both committed and both drift-checked in CI ([ADR-0080](../adr/0080-a-generated-client-is-committed-and-the-document-must-earn-it.md)). **The API row is complete** |
...
| Licensing and supply chain | **Built** | Since 2026-09-22 every one of the nine distributed artifacts carries the licence notice of what is in it and publishes its own CycloneDX SBOM (`REQ-CON-013`, `REQ-CON-010`, [ADR-0083](../adr/0083-the-notice-travels-inside-the-artifact.md)). The notices are generated by [`tools/notices.py`](../../tools/notices.py) from what each artifact holds, committed so a new licence shows up in the pull request that brings it, and checked in CI; the six `scratch` images have theirs **compiled into the binary**, because they have no filesystem to hold a file. The view is `GET /api/v1/version/notices`, public and rendered in the client beside the version. `ThirdPartyNoticesTest`, `NoticesEndpointIT`, `notices.test.ts`, `sbomTravels`. *One SBOM stood for nine artifacts, and no notice existed at all, until that date* |
