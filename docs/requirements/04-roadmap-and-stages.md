...
| First-party plugins | **Not built** | `plugin-smtp`, `plugin-blobstore-s3`, `plugin-blobstore-nextcloud`, `plugin-webhook`, `plugin-oidc`. They live in [`plugins/`](../../plugins/) in this repository, each in the language that fits it ([ADR-0072](../adr/0072-first-party-plugins-live-here.md)). Everything that leaves the deployment waits on them: e-mail, webhooks, OIDC federation and the two remote media stores |
