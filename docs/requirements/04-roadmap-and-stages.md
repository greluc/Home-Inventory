...
| First-party plugins | **One of five** | [`plugins/webhook/`](../../plugins/webhook/) is written — Rust, `scratch`, signed payloads, no redirects followed, and its own unit tests including the RFC 4231 vectors for the signature. `plugin-smtp`, `plugin-blobstore-s3`, `plugin-blobstore-nextcloud` and `plugin-oidc` are not. **None can be declared in a deployment yet**: `deploy/services.yaml` has no plugin service and the generator produces no per-plugin segment, allowlist or plugin list, which is the other half of `REQ-PLG-013` ([ADR-0072](../adr/0072-first-party-plugins-live-here.md)) |
