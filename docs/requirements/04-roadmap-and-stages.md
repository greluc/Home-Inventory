...
| Media | **Partly** | Reference counting, the fail-closed malware scan, derivatives and the signed URLs are built (`MediaListingIT`, `MediaScanIT`, `DerivativeGenerationIT`, `OrphanedBlobSweepIT`). **Resumable upload (tus) is not** (`REQ-MED-008`) — the 2026-09-14 survey said it was, and nothing in the tree ever implemented it. The **S3 and Nextcloud adapters are plugins** and are written, with the port they plug into wired at the same time: a tenant that grants one has its next bytes written there and none of its older ones moved (`TenantBlobStoreIT`, [ADR-0074](../adr/0074-granting-a-store-routes-and-does-not-move.md)) |
