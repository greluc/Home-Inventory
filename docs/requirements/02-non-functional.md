...
| REQ-NFR-009 | The system runs within 8 GB RAM in the `standard` profile and within 4 GB in `minimal`, **measured as the sum of memory reservations** — ≈ 5.4 GB and ≈ 3.4 GB respectively. The sums of *limits* (≈ 9.4 GB and ≈ 6.9 GB) deliberately over-commit, because each limit is sized for that service's own worst case and those do not coincide ([06 §6.7](../architecture/06-deployment-view.md)). The mandatory malware scanner ([ADR-0024](../adr/0024-malware-scan.md)) and the egress proxy it needs ([ADR-0036](../adr/0036-scanner-egress.md)) are included in both figures. | M | 1 | Both profiles started and verified in CI under a steady load, with resident memory measured against the reservation sums — not against the limit sums, which the design states as over-committed |
