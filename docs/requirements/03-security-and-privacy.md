...
| REQ-SEC-103 | **The forwarded-header chain covers both hops.** Since [ADR-0042](../adr/0042-edge-is-not-internal.md) there are two proxies in front of `api` — the operator's reverse proxy and `web` — so `HOMEINV_TRUSTED_PROXIES` must contain **both**, and `web` must **append** to `X-Forwarded-For` rather than overwrite it. `REQ-SEC-063` described one hop. | 0 | A startup check fails when `web`'s `internal` address is absent from the list. Three tests: the client IP reaching `api` is the **client's**, not `web`'s — otherwise per-IP rate limiting (`REQ-SEC-064`) throttles every tenant as one and every audit entry records one source (`REQ-SEC-068`); a forged `X-Forwarded-For` from a client is **not** believed; and removing `web` from the list makes the first test fail rather than pass quietly |
