...
| **ClamAV** | **Uploads are still accepted** — they are answered `202` before the scan runs ([ADR-0054](../adr/0054-the-scan-is-asynchronous.md)) — and none of them becomes retrievable: each ends in `SCAN_FAILED`, and `GET /api/v1/media/{id}` answers `503` until the retry queue reaches a verdict. *This row said "uploads are rejected (`503`)" until 2026-09-12, which was true of the implementation and not of the design.* Already accepted blobs stay unretrievable; a background run catches the scan up. The only degradation level that blocks a user function entirely — deliberately, see [ADR-0024](../adr/0024-malware-scan.md) | everything except uploads |
...
| Catch-up scan for objects with no verdict | every 5 min, up to 12 times per object | Release or discard uploads accepted during a scanner outage. **Not a scheduled sweep**: a query for "every object with no verdict" spans tenants, and `homeinv_app` has neither `BYPASSRLS` nor a way to enumerate tenants, so the retry travels on the broker and carries its own tenant ([ADR-0054](../adr/0054-the-scan-is-asynchronous.md)) |
