...
| `HOMEINV_NOTIFICATION_INTERVAL_MS`, `_REMINDER_`, `_EXPORT_`, `_IMPORT_`, `_BLOB_SWEEP_`, `_AUDIT_ANCHOR_` | no (`30000`, `3600000`, `30000`, `30000`, `604800000`, `900000`) | How often the `worker` runs each of its six queues — delivering notifications, raising reminders (`REQ-NOTI-001`), building an export (`REQ-PORT-005`), reading an uploaded archive (`REQ-PORT-003`), removing blobs nothing points at (`REQ-MED-011`) and anchoring the audit chain (`REQ-SEC-070`). All six are a **fixed delay** measured from the end of the last run rather than a rate, so a run that took longer than its interval does not have a second one starting on top of it. `api` reads them and acts on none: every one of these is `worker`-only, and the two containers share a database and nothing else |
