...
| S10 | **`api` or `worker`** attempts to reach any host outside the deployment | **The connection is refused** — there is no route and no configured target ([ADR-0026](../adr/0026-core-outbound-via-plugins.md)) | A CI network test against `api` and `worker`. It read *"every core container"* until 2026-09-11, which is a test with two members whose expected result is wrong: `web` and `egress-proxy` reach out by design ([ADR-0042](../adr/0042-edge-is-not-internal.md), `REQ-SEC-102`), and a failing test with a wrong expectation is one that gets weakened until it passes. It is still the test that would have caught the contradiction this scenario was written for |
...
| S12 | A plugin holding **no capability at all** tries the core's management port, the scanner, or another plugin | **Every attempt is refused** — it is on its own segment, and the management listener binds to `internal` | The connectivity test, extended: two plugins, each trying the other, `clamd`, and `:8090` on both core roles ([ADR-0037](../adr/0037-per-plugin-network-segments.md), REQ-SEC-098/099) |
| S14 | **`web` is compromised** — the one container with a published port | It reaches `api` and nothing else. `postgres`, `valkey`, `rabbitmq`, `opensearch`, `blobstore` and `:8090` on both core roles all refuse, and each of them would demand a credential `web` does not hold in any case | The connectivity suite, extended ([ADR-0044](../adr/0044-internal-is-not-a-trust-boundary.md), REQ-SEC-104/105). Until 2026-09-11 `web` was a full member of `internal` and this scenario had no entry — the suite asserted six properties, all about plugins |
| S13 | The instance runs for a month without a successful ClamAV signature fetch | The 48-hour staleness alert fires **and** the proxy's access log names the failed fetch, so the cause is visible rather than only the symptom | The `minimal` profile is started in CI and must complete one signature fetch through the proxy ([ADR-0036](../adr/0036-scanner-egress.md), REQ-SEC-093) |
...
| R17 | **iOS scanning rests permanently on a WASM decoder.** `BarcodeDetector` is not implemented in WebKit, so on every iPhone and iPad the browser path is ZXing-WASM — on exactly the device class Q6 is written around (the cellar, the phone, one hand). The corpus called it a *fallback* and claimed "Safari 17+" until 2026-09-11. | Scanning is slower and hungrier on the platform where it matters most, and a WASM regression is felt by half the users | medium | The decoder runs in a web worker and its performance is a **measured** target on iOS, not an assumption (`REQ-IDENT-006`, scenario U3). The KMP iOS app reaches AVFoundation natively and is the structural answer — the trigger for pulling it forward from stage 3 is a measured scan-to-result above 1 s on current iPhone hardware | U3 measured on iOS drifts above 1 s, or bug reports cluster on iPhone |
| R16 | **A guarantee is written in one document and delivered in another, and nobody compares them.** Not a hypothetical: on 2026-09-11 a second review pass found a fail-closed scanner with no route to its signature mirror, a "not publicly reachable" management port readable by any plugin, "its own network segment" that was one shared segment, a CSP nonce nothing could mint, and a QR argument the QR itself refuted. Each was a sentence that was true where it was written and false where it was implemented. | Properties the corpus claims and the topology does not provide — the most expensive class of defect, because it reads as settled | **high** | Every load-bearing property gets a **CI test that exercises it**, not a document that asserts it: the connectivity test, the isolation proof, the header comparison, the drift check. The rule is that a property with no test is a claim. The review that found these is repeated per stage | A document says "enforced" or "cannot" and no test name appears next to it |
...
| **Chain truncation** | The append-only record of where a tenant's audit chain legitimately begins after a retention run, so a recorded deletion is distinguishable from tampering ([ADR-0046](../adr/0046-truncatable-audit-chain.md)) |
| **Cursor** | An opaque, signed pointer for pagination (API) resp. reconciliation position (sync), signed with `HOMEINV_URL_SIGNING_KEY_FILE` |
...
| **`frontend`** | The two-member network segment holding `web` and `api` and nothing else, so the ingress reaches the API and no datastore ([ADR-0044](../adr/0044-internal-is-not-a-trust-boundary.md)) |
...
| **WAL archive** | The `pgwal` volume PostgreSQL archives write-ahead log segments into; without it the recovery point objective is the last daily dump ([ADR-0045](../adr/0045-wal-archive-volume.md)) |
