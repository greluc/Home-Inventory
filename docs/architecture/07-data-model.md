...
2. **Every domain table carries `tenant_id`** and is subject to row-level
   ...
   | `identity.federated_identity` | A federated sign-in happens **before** any tenant is known, exactly as a password one does: the person arrives at a login page and which tenants they belong to is what the session goes on to decide (`REQ-AUTH-005`). `identity.app_user`'s reason applied to the foreign half of the same account | Keyed on `(issuer, subject)` and on nothing else, unique instance-wide: a second account on the same foreign identity would make the sign-in ambiguous, and an ambiguity resolved by `LIMIT 1` is an account takeover with an ordering bug in front of it. The address is stored as **evidence of what it was at linking** and nothing looks an account up by it, which is `REQ-AUTH-006` in the schema. Read by `(issuer, subject)` during a sign-in or by `user_id` for the caller's own account page; no endpoint takes somebody else's user id |
   | `identity.federated_login` | The sign-in **in progress**, for the same reason one step earlier: the flow begins before there is a session at all. It exists because [ADR-0029](../adr/0029-session-cookie-and-oidc-state.md) keeps the session cookie `SameSite=Strict`, so nothing written into a cookie before the provider's redirect comes back with it | Only the **hash** of the handle is stored, the treatment `identity.password_reset` gives a reset token; the handle itself lives in the `state` parameter and nowhere else. Single use through `consumed_at`, which is what makes a replayed callback fail. `purpose` is fixed before the redirect and a callback cannot change it — the difference between linking an account and taking one over — and `return_to` is constrained to a local path, because an absolute one would be an open redirect with a sign-in in front of it |
...
4. **Every domain table carries `version bigint`** for optimistic locking, plus
   ...
   **A *domain* table is one that holds mutable state a user edits.** Five kinds
   ...
   | **Append-only records of an event** | `inventory.maintenance_entry`, `sync.change_log`, `audit.audit_entry`, `audit.chain_anchor`, `audit.chain_truncation` ([ADR-0046](../adr/0046-truncatable-audit-chain.md)), `audit.revision_record`, `notification.delivery_attempt`, `notification.security_delivery_attempt`, `notification.reminder`, `identification.label_base_url_usage`, `tenancy.erasure_certificate` | Nothing updates them, so `version`, `updated_at` and `updated_by` would be dead columns. `occurred_at` and the actor are already in the row, under the names the record uses. `revision_record` is the history *of* an edit — a row per change with the state after it — and editing the history is the one thing it must not permit. A `maintenance_entry` is the same argument about a thing rather than about a row: `REQ-LIFE-003` says entries are not retroactively editable, so a correction is a second entry and the table grants no `UPDATE`. A `reminder` is the record that a rule already said something (`REQ-NOTI-001`): it is written once and read back only to keep the rule from repeating itself, and a reminder that should not have gone out is not un-sent by editing the row |
   | **Infrastructure** | `outbox.event_publication`, `idempotency.processed_request`, `crypto.tenant_data_key`, `notification.notification`, `notification.security_notification`, `identity.password_reset`, `portability.export_job`, `portability.import_job`, `portability.import_provenance` | Owned by a mechanism, not by a block's domain model. A data key is issued and retired, never edited. A queued notification is a unit of work: its `state` and `next_attempt_at` are the delivery mechanism's bookkeeping, and an `updated_by` on it would name the scheduler rather than a person. The account queue beside it is the same shape with no tenant ([ADR-0066](../adr/0066-instance-level-capability-grants.md)), and a reset token is the same shape again: `used_at` is the spend, nothing else about the row ever changes, an `updated_by` would name whoever presented the token — who is anonymous at that moment, which is the property `REQ-SEC-018` rests on — and the single-use guarantee comes from that column and a partial unique index rather than from an optimistic lock. An `export_job` is the same shape once more: `state` and `progress` are the build's own bookkeeping and an `updated_by` on them would name the worker, while who **asked** is a fact about a person and is a column — and an `import_job` is its mirror, carrying the report of what one upload did rather than anything a person edits. `import_provenance` is append-only beside it: a second import of the same file writes a second row rather than editing the first, which is what makes the table the history `REQ-PORT-008` asks the provenance to be visible in |
...
| `tenancy.set_tenant_lifecycle_state` | Suspending a tenant is an **operator's** act and the operator is a member of no tenant (`REQ-SEC-082`, [ADR-0057](../adr/0057-the-instance-operator.md)) | Moves between `ACTIVE` and `SUSPENDED` and refuses every other state in both directions: an erasure is the tenant's own decision with a grace period, and withdrawing one is what the revocation token is for (`REQ-TEN-011`). Owned by `homeinv_tenant_state`, a role that may reach this one column and nothing else |
| `tenancy.lifecycle_state_of_tenant` | The operator's view of the same | All four states, because an operator looking at a tenant that is being erased should see that — which is exactly why the setter refuses it |
...
| `media.tenants_with_expired_uploads` | The quarter-hourly sweep looks for the tenants holding an upload that was begun and never finished, so there is no context to set yet (`REQ-MED-008`) | Returns tenant ids and nothing else; the sessions are read and removed under each tenant's own context, and their staged bytes go first |
| `portability.tenants_with_queued_exports` | The export run looks for the tenants that have asked for one, so there is no context to set yet (`REQ-PORT-005`) | Returns tenant ids and nothing else; the jobs are claimed and built under each tenant's own context |
| `inventory.tenants_with_depreciable_items` | The nightly depreciation run is looking for the tenants that have something to depreciate, so there is no context to set yet (`REQ-LIFE-009`) | Returns tenant ids and nothing else — not a row, not a count, not an amount. The condition is deliberately loose, so that deciding whether a type actually declares a useful life happens inside the tenant's own context where the catalogue is readable |
| `portability.tenants_with_queued_imports` | The same, for the other direction: an uploaded archive is read back by the worker, which has no context until it knows whose job it is (`REQ-PORT-003`) | Returns tenant ids and nothing else; the archive is read and written under the receiving tenant's own context |
...
| `media.upload_session` | An upload still arriving (`REQ-MED-008`) | Holds what the blob store does not know — who the upload is for, what it will hang on, when it stops being worth keeping — and **not the offset**, which is the length of the staged file: a number kept in two places disagrees after a crash, and the one on disk is the one that is true ([ADR-0084](../adr/0084-an-upload-arrives-in-pieces-and-is-staged-where-the-volume-is.md)) |
...
| `inventory.item` — the lifecycle and disposal columns | Where it is in its life, and how it left | `lifecycle_state` is a **closed set** since 2026-09-20 — `ACTIVE`, `LENT`, `ARCHIVED`, `TRASHED`, `SOLD`, `DISPOSED` — because [11 §11.3](11-offline-synchronisation.md) *ranks* these to resolve a sync conflict, and it was free text until then. `PURGED` is absent: a purge is a `DELETE` and leaves no row. `(lifecycle_state = 'TRASHED') = (deleted_at IS NOT NULL)` keeps the state and the timestamp from drifting apart — the state says what, the timestamp says when, and REQ-CORE-009's retention run needs the second. Five disposal columns (`disposal_amount`/`_currency`, `disposed_on`, `disposal_recipient`, `disposal_note`, `REQ-LIFE-007`): **columns and not a table**, because a thing is sold once and the sequence that makes `maintenance_entry` a table does not exist here. `disposed_on` is present exactly when the state says the item is gone, and an amount is allowed **only** against `SOLD` — a price on a `DISPOSED` row would be a row saying both. `disposal_recipient` is free text and not a member reference, for `loan.borrower_user_id`'s reason one step further on: things are sold to strangers |
...
| `inventory.maintenance_entry` | What was done to an item, and when | One row per piece of work (`REQ-LIFE-003`): the date it was **done** rather than typed, the kind in the tenant's own words, what it cost as an amount with its currency — or nothing at all, because work under warranty costing nothing and work costing 0.00 are different claims — and a note. **Append-only**: no `version`, no `updated_*`, and `homeinv_app` is granted no `UPDATE`, so the log cannot be tidied up afterwards. The invoice is a media object whose target is the entry, which is why there is no attachment column here |
| `inventory.loan` | Lending | Borrower (internal or free text), handed out, due back, returned (`REQ-LIFE-005`). The borrower is **one of two things and never both** — a member of this tenant or a name somebody typed — in two nullable columns with `num_nonnulls(...) = 1`, the shape `tagging.tag_assignment` has for the same reason. `borrower_user_id` carries **no** foreign key: `identity.app_user` is instance-wide so the reference could not be composite, and somebody who leaves the household still borrowed the drill in March. **At most one open loan per item**, by a partial unique index on `(tenant_id, item_id) WHERE returned_on IS NULL` — the instrument `identity.password_reset` uses to keep one reset open per account, and the reason “is it lent” has a single answer rather than a flag on `item` that can fall out of step. Unlike `maintenance_entry` a loan **is** edited: the return is written into the row that recorded the handover, because a return is the end of *that* loan and not a separate event, so it carries `version` and all four audit columns. A lent item cannot be trashed (`ItemLentException`, `409 item-lent`), which is what REQ-LIFE-005's “not deletable” means: the row is the only record of who to ask for it back |
| `notification.notification_rule` | A reminder rule, as data | `REQ-NOTI-001`'s three parts plus the one an offset cannot do without: a **trigger kind** naming which date or condition, a saved search narrowing *which* things (`ON DELETE SET NULL`, so removing the smart list widens the rule rather than silently stopping it), an `offset_days` where **positive warns early and negative waits until after** — one column covering both a warranty warning and REQ-LIFE-006's overdue loan — and a channel key. The trigger set is closed by a check constraint and mirrored by `ReminderTrigger`, because a value that reaches a stored row outlives the constant that produced it |
| `notification.reminder` | What a rule has already raised | Append-only (7.1 rule 4), and the whole idempotency of the feature: `UNIQUE (tenant_id, rule_id, subject_kind, subject_id, due_on)`. The run **inserts and catches the duplicate** rather than reading first and writing after — two overlapping passes would both find nothing recorded and both send, and a reminder that repeats is one people switch off. `due_on` is the date the reminder was *about* rather than the day it went out, which is what makes a moved due date raise a new reminder and an unchanged one raise none. The subject is a `(kind, id)` pair with no foreign key — the one place this chapter allows it, because nothing joins on it: the subject is an item today and a stocktake run tomorrow, and a column per trigger would be a migration per trigger |
