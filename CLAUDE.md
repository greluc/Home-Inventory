...
- **Requirements** live in [`docs/requirements/`](docs/requirements/README.md) as
  `REQ-<AREA>-NNN` (429 of them). IDs are stable and never reused; a dropped requirement
...
- **`api/clients/kotlin/` is the one Kotlin in the repository** — the generated
  Multiplatform client of `REQ-API-002`, a Gradle subproject `:api-client-kotlin`
  built by `./gradlew build`. Everything under its `src/` is generated by
  `tools/kotlin_client.py` from `api/openapi.yaml` and committed; its
  `build.gradle.kts` is not, and names its dependencies through the version
  catalogue. The TypeScript client is generated too and lives in
  `web/src/generated/`, beside the one thing that consumes it
  ([ADR-0080](docs/adr/0080-a-generated-client-is-committed-and-the-document-must-earn-it.md)).
  **Neither is hand-edited**, and CI fails while either disagrees with the document.
