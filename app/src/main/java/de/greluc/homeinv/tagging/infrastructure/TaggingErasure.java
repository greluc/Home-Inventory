 ...
 *
 * <p>Assignments before tags, tags before the groups they sit in.
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class TaggingErasure implements TenantErasure {
  ...
  private static final List<String> DELETES =
      List.of(
          ...
          "delete from tagging.tag",
          "delete from tagging.tag_group");
   ...
   * {@inheritDoc}
   *
   * <p>Before {@code inventory} and {@code locations}: {@code tagging.tag_assignment} references
   * both. The references cascade, so the database would not refuse the other order — it would
   * silently remove the assignments as part of another block's delete, and this block would then
   * report a number that was not the truth about what it held.
   */
  @Override
  public int order() {
    return 20;
