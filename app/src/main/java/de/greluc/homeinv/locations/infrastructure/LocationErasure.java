 ...
 *
 * <p>One table, and its own self-reference is handled by deleting the lot in one statement: a
 * parent and its children go together, so no row is ever left pointing at one that is gone.
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class LocationErasure implements TenantErasure {
   ...
   * {@inheritDoc}
   *
   * <p>Before {@code catalog}: {@code locations.location} references the category it was created
   * as, and the version of that category it was last saved against (V5, V14).
   */
  @Override
  public int order() {
    return 50;
