 ...
 *
 * <p>Rules and grants before the definitions they belong to. The schema is {@code authz} and not
 * {@code authorization}, for the reason ADR-0058 gives.
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class RoleErasure implements TenantErasure {
  ...
  private static final List<String> DELETES =
      List.of(
          "delete from authz.field_visibility",
          "delete from authz.role_permission",
          "delete from authz.role_definition");
   ...
   * {@inheritDoc}
   *
   * <p>After {@code tenancy}: a membership references the tenant-owned role it grants (V26), and
   * a role definition still held by a member cannot be removed — which is the same refusal that
   * stops an administrator deleting a role somebody is using.
   */
  @Override
  public int order() {
    return 70;
